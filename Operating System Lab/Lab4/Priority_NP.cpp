...
struct process
{
	int pr,pt,at,bt,ct,tat,wt;
};
struct process p[10],temp;

void sort(int lb,int ub,int ch)
{
	// ch Based on 1.Arrival Time 2.Priority
	int i,j;
	for(i=lb;i<ub;i++)
	{
		for(j=i+1;j<=ub;j++)
...
{
	printf("PR PT AT BT CT TAT WT\n");
	for(int i=0;i<n;i++)
	{
		printf("%d  %d  %d  %d  %d  %d  %d\n",p[i].pr,p[i].pt,p[i].at,p[i].bt,p[i].ct,p[i].tat,p[i].wt);
	}
